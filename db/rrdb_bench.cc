...
DEFINE_string(benchmarks,
              ...
              "\tfillseq_rrdb          -- rrdb write N values in sequential key order\n"
              "\tfillrandom_rrdb       -- rrdb write N values in random key order\n"
              "\tfilluniquerandom_rrdb -- rrdb write N values in unique random key order\n"
              "\treadrandom_rrdb       -- rrdb read N times in random order\n"
              "\tdeleteseq_rrdb        -- rrdb delete N keys in sequential order\n"
              "\tdeleterandom_rrdb     -- rrdb delete N keys in random order\n"
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run() {
    ...
    PrintHeader();
    if (FLAGS_benchmarks.find("_rrdb") == std::string::npos) {
      Open(&open_options_);
    }
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "filluniquerandom_rrdb") {
        if (num_threads > 1) {
          fprintf(stderr,
                  "filluniquerandom_rrdb multithreaded not supported"
                  ", use 1 thread");
          num_threads = 1;
        }
      ...
      } else if (name == "deleteseq_rrdb") {
        method = &Benchmark::DeleteSeqRRDB;
      } else if (name == "deleterandom") {
        method = &Benchmark::DeleteRandom;
      } else if (name == "deleterandom_rrdb") {
...
int main(int argc, char** argv) {
  ...
  sleep(1);
  fprintf(stdout, "Init rrdb succeed\n");
